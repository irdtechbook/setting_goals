...
課題を解決する方法は人それぞれありますが、私が実行している@<b>{投機的実行}方法はすべての方に適用できるとは思いませんが、この方法は技術を広く知ることになるので技術の流行り廃りに対して強いです。さらに技術の幅が広がるので自分の技術の特性を知るよい機会になります。このように、投機的実行はメリットが多いです。将来の技術習得に疑問を感じた方は読んでくだされば幸いです。
...
== 予測が外れても将来必要になることもある
私がIT業界に転職したとき（2000年）、世の中はiモード携帯がブームになりつつあるころで、技術はPerlが中心の時代でした。Perlは今も使われている言語ですが、いつかは中心から外れてしまい廃れてしまっては困ると考え、私はここで投機的実行を行いました。
...
調査会社が出した「今年トレンドの開発言語」とかは指標として見るのはよいですが、これをメインの予測データとして使うのはオススメしません。調査対象が全然関係ない業界だったり、GitHubのコミット数だったりと、的外れなデータが使われています。とくにGitHubに関しては、調査対象がPublic Repositoryのみになっており、OSSの調査ならハズレではないものの、本当に予想に使う場合は企業が使用しているRepositoryまで含めないと意味がないからです。
